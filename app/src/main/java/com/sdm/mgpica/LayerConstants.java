package com.sdm.mgpica;

// Created by TanSiewLan2021
// Edited by Tan Sze Ting
// Edited by Timothy

// Able to help you do layering of different assets rendered on the screen

public class LayerConstants {

    public final static int BACKGROUND_LAYER = 0;
    public final static int RENDERSMURF_LAYER = 1;
    public final static int STAR_LAYER = 3;
    public final static int RENDERPROJECTILE_LAYER = 5;
    public final static int TERRAIN_LAYER = 100;  // Example
    public final static int RENDERPLAYER_LAYER = 101;
    public final static int RENDERBUTTON_LAYER = 150;
    public final static int RENDERTEXT_LAYER = 151;
    public final static int UI_LAYER = 1000;
}